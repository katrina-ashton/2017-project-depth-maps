function meas = generate_meas(type, O, rot, nmeas, varargin)
if strcmp(type,'cylinder')
    ...
    nr = int32(nmeas/((Ac/Ar)+1)); %Number of points in rectangle
    nc = int32((nmeas - nr)/2);    %Number of points in each circle
    ...
    meas = [theta, phi, r];
    meas = new_scoord(meas(:,1), meas(:,2), meas(:,3), O, rot);
