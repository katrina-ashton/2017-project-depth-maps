function [stats_full_avg, stats_approx_avg, stats_best_avg] = ...
                                       expBestApproxBTM( platonic_solid, ...
                                                         basis, ...
                                                         signal_file, ...
                                                         level, ...
                                                         num_samples, ...
...
%                     empty string is provided then no result is stored
  ...
  for i = 1 : num_samples;
    ...
    btm_approx = smartApproxBTM( btm, coeffs_rotated, coeffs_retained_signal);
...
% @param  k  number of coefficients to retain
  ...
  disp( sprintf( 'Approx :: nnz( btm_approx) / k = %i / %i', nnz( btm_approx), k));
  
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function btm_approx = smartApproxBTM( btm, coeffs, coeffs_retained)
%
%

  coeffs_retained = coeffs_retained * coeffs_retained;

  indices = find( mean(coeffs) == 0);
  btm( : , indices) = 0;

  abs_btm_weighted = abs( btm .* repmat( mean( coeffs), size(btm,1), 1));
  
  % weighting by coefficients automatically discards rows where the
  % corresponding basis function coefficient is zero
  slist = sort( abs_btm_weighted(:));
  threshold = (slist( numel(slist) - coeffs_retained) ...
             + slist( numel(slist) - coeffs_retained - 1)) / 2;
                
  indices = find( abs_btm_weighted < threshold);
              
  btm_approx = btm;
  btm_approx( indices) = 0;
  
  disp( sprintf( 'Best :: nnz( btm_approx) / k = %i / %i', ...
        nnz( btm_approx), coeffs_retained));
  
end 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function btm_approx = smartApproxBTM2( btm, coeffs, coeffs_retained)
%
%
